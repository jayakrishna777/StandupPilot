...
- [x] Development and isolated test databases or schemas use server-side `DATABASE_URL` settings.
...
- [x] A proposal issues at most one Jira transition across repeated calls or concurrent approvals.
...
- [x] A fictional Jira issue passes a real read, transition discovery, exact transition, post-write verification, and reset cycle.
...
- [x] Handoff reports the branch, final commit, commands run, results, PostgreSQL and Jira evidence, and remaining limitations.
